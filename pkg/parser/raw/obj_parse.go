...
func parseObjectForImages(obj runtime.Object) []string {
	...
	case "Pod":
		pod := obj.(*corev1.Pod)
		log.Println("Found Pod:", pod.GetName())
	...
	case "Deployment": // only supports appsv1 and v1beta1 for now
		...
		case "v1":
			...
			log.Println("Found Deployment:", deployment.GetName())
		...
		default:
			log.Println("Skipping non apps/v1 or apps/v1beta1 Deployment object")
		}
	case "StatefulSet": // only supports apps/v1
		ss, ok := obj.(*appsv1.StatefulSet)
		if !ok {
			log.Println("Skipping non apps/v1 StatefulSet object")
			return images
		}
		log.Println("Found StatefulSet:", ss.GetName())
	...
	case "Job": // only supports batch/v1
		...
		if !ok {
			log.Println("Skipping non batch/v1 Job object")
			return images
		}
	...
	case "CronJob": // only supports batch/v1beta1
		...
		if !ok {
			log.Println("Skipping non batch/v1betav1 CronJob object")
			return images
		}
		log.Println("Found CronJob:", job.GetName())
		if imgs := parseImagesFromContainers(job.Spec.JobTemplate.Spec.Template.Spec.Containers); len(imgs) > 0 {
			images = append(images, imgs...)
		}
	default:
		// log.Println("Skipping non-container based object:", gvk.Kind) // TODO: verbose logging
	}
